...
SELECT DISTINCT T1.STATE , T4.Manufacturer_Name, T1.Country,SUM(T2.Quantity) AS TOTAL_QUANTITY 
FROM DIM_LOCATION AS T1
LEFT JOIN FACT_TRANSACTIONS AS T2 ON T1.IDLocation = T2.IDLocation
...
WHERE T4.Manufacturer_Name = 'Samsung' AND T1.Country = 'US'
...
SELECT T1.ZipCode, T1.State, T3.Model_Name ,COUNT(T2.DATE) AS NO_OF_TRANSACTION
...
HAVING Unit_price = MIN(Unit_price)
ORDER BY Unit_price
...
SELECT  T3.Manufacturer_Name,T1.Model_Name,AVG(T2.TotalPrice) AS AVERAGE_PRICE
...
HAVING T3.Manufacturer_Name IN (
								SELECT TOP 5 T1.Manufacturer_Name  FROM DIM_MANUFACTURER AS T1
...
SELECT T1.Customer_Name , YEAR(T2.Date) AS Year_Of_Date , AVG(T2.TotalPrice) AS AVERAGE_AMT_SPENT
...
SELECT * FROM (
				SELECT TOP 5 T1.IDModel,T2.Model_Name FROM Fact_Transactions T1
...
SELECT X.Manufacturer_name AS IN_2009,Y.Manufacturer_name AS IN_2010
FROM(
     SELECT Manufacturer_name,DATEPART(Year,date) AS YEAR,SUM (TotalPrice) AS TOTAL_SALE,
	 ROW_NUMBER() OVER(ORDER BY SUM (TotalPrice) DESC) AS ROW_NO
...
ON (X.ROW_NO=Y.ROW_NO)  AND (X.ROW_NO='2' AND Y.ROW_NO = '2')



--SELECT TOP 2 T1.MANUFACTURER_NAME,DATEPART(Year,date) AS YEAR,SUM(T3.TOTALPRICE) AS TOTAL_SALE
...
--GROUP BY T1.MANUFACTURER_NAME,DATEPART(Year,date)
...
SELECT DISTINCT T4.Manufacturer_Name ,T2.YEAR FROM FACT_TRANSACTIONS AS T1  
INNER JOIN DIM_DATE AS T2 ON T1.Date=T2.DATE
INNER JOIN DIM_MODEL AS T3 ON T3.IDModel=T1.IDModel 
INNER JOIN DIM_MANUFACTURER AS T4 ON T4.IDManufacturer=T3.IDManufacturer
WHERE T2.YEAR =2010 and  T4.Manufacturer_Name  NOT IN
								(								
								SELECT t4.Manufacturer_Name  FROM FACT_TRANSACTIONS AS T1  
								INNER JOIN DIM_DATE AS T2 ON T1.Date=T2.DATE
								INNER JOIN DIM_MODEL AS T3 ON T3.IDModel=T1.IDModel 
...
SELECT Customer_Name, YEAR(DATE) AS YEAR, SUM(T1.TOTALPRICE) TOTAL_SPEND, AVG(QUANTITY) AS AVG_QTY, AVG(TOTALPRICE) AS AVG_SPEND,
	   (AVG(TOTALPRICE) - LAG(AVG(TOTALPRICE),1) OVER (PARTITION BY Customer_Name ORDER BY YEAR(DATE))) * 100 / AVG(TOTALPRICE) as [%CHANGE]
FROM FACT_TRANSACTIONS T1
...
SELECT TOP 100 DATEPART(yyyy, ft.date) Year,dc.Customer_Name,AVG(ft.[TotalPrice]) avg_spends,AVG(ft.Quantity) avg_qty
FROM FACT_TRANSACTIONS ft
INNER JOIN dim_customer dc ON ft.IdCustomer = dc.IdCustomer
GROUP BY DATEPART(yyyy, ft.date),ft.IdCustomer,dc.Customer_Name
ORDER BY SUM(ft.quantity) DESC

--percentage change in average spend

SELECT DATEPART(yyyy, ft1.date) AS change_year,
((AVG(ft1.[TotalPrice]) - AVG(ft2.[TotalPrice])) / AVG(ft2.[TotalPrice]))*100 AS Change
FROM FACT_TRANSACTIONS ft1
INNER JOIN FACT_TRANSACTIONS ft2
  ON DATEPART(yyyy, ft1.date) = (DATEPART(yyyy, ft2.date) + 1)
GROUP BY DATEPART(yyyy, ft1.date)
